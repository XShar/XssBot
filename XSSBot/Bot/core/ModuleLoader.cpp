...
HMODULE __stdcall LoadModule(char* module)
{
	...
	my_memcpy(&DosHeader, module, sizeof(IMAGE_DOS_HEADER));
	if (DosHeader.e_magic == IMAGE_DOS_SIGNATURE) // проверим сигнатуру
	{
		...
		{
			my_memcpy(&tmp[0], (char*)(module + sizeof(IMAGE_DOS_HEADER)), DosHeader.e_lfanew - sizeof(IMAGE_DOS_HEADER));
		}
		...
		// считаем заголовок
		my_memcpy(&PeHeader, module_pheader, sizeof(IMAGE_NT_HEADERS));

		if (PeHeader.Signature == IMAGE_NT_SIGNATURE) // проверим сигнатуру
		{
			// считаем 10 секций
			my_memcpy(&Section[0], (char*)(module_pheader + sizeof(IMAGE_NT_HEADERS)), sizeof(IMAGE_SECTION_HEADER) * PeHeader.FileHeader.NumberOfSections);
			...
			if (retadr) // если память выделилась
			{
				...
				my_memcpy((void*)retadr, &DosHeader, sizeof(IMAGE_DOS_HEADER));
				// скопируем туда PE заголовок
				my_memcpy((void*)(retadr + DosHeader.e_lfanew), &PeHeader, sizeof(IMAGE_NT_HEADERS));
				// скопируем туда таблицу секций
				my_memcpy((void*)(retadr + DosHeader.e_lfanew + sizeof(IMAGE_NT_HEADERS)), &Section[0], sizeof(IMAGE_SECTION_HEADER) * PeHeader.FileHeader.NumberOfSections);

				// если есть Rich данные то и их тоже скопируем
				if (sizeof(IMAGE_DOS_HEADER) < DosHeader.e_lfanew)
				{
					my_memcpy((void*)(retadr + sizeof(IMAGE_DOS_HEADER)), &tmp[0], DosHeader.e_lfanew - sizeof(IMAGE_DOS_HEADER));
				}
				...
				for (int i = 0; i < PeHeader.FileHeader.NumberOfSections; i++)
				{
					...
					// считаем всё секцию
					my_memcpy((void*)(retadr + Section[i].VirtualAddress), PointerToRawData, Section[i].SizeOfRawData);
				}
